    ...
    try {
        ...
        if(!name || !email || !password){
            return response.status(400).json({
                message : "Provide your name, email and password",
                error : true,
                success : false
            })
        }
...
export async function verifyEmailController(request,response){
    try {
        ...
        const user = await response.UserModel.findOne({_id : code})
        ...
        const updateUser = await response.UserModel.findOne({_id : code},{
            verify_email : true
        })
